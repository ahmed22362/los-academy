      ...
      <div className={"px-5 py-4"}>
        <Table>
          <Table.Head theme={customTheme.head}>
            ...
            <Table.HeadCell theme={customTheme.head}>Age</Table.HeadCell>
            <Table.HeadCell theme={customTheme.head}>Added on</Table.HeadCell>
            <Table.HeadCell theme={customTheme.head}>Status</Table.HeadCell>
            <Table.HeadCell theme={customTheme.head}>Download</Table.HeadCell>
          </Table.Head>
          <Table.Body className="divide-y">
            {isLoading ? (
              ...
              displaydMatrial.map((matrial: any, index: number) => {
                return (
                  <FetchMatrialData
                    key={index}
                    matrialData={matrial}
                    updateComponent={fetchAllMatrials}
                  />
                );
              })
            ) : (
              <tr>
                <td className="p-5">There is No Matrials</td>
              </tr>
            )}
          </Table.Body>
        </Table>
        <div className="card mt-4">
          <Paginator
            first={first}
            rows={rows}
            totalRecords={allMatrial.length}
