...
function TeacherScheduleData({data}: {data: any}) {
    ...
    const sessionDate = convertDate(session.sessionDate, "UTC", Intl.DateTimeFormat().resolvedOptions().timeZone, "MM/DD/YYYY hh:mm A")
  ...
  return (
        <div className={"py-5 px-4 bg-white-color rounded-[16px] my-4 flex gap-3 font-semibold flex-wrap"}>
            ...
            <p>{`with ${userName}`}</p>
            <button className='smallBtn hover:bg-secondary-hover transition-colors ms-auto'
