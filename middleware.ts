...
const protectedStudentRoutes = ["/student_profile"];

const userLoginRoutes = ["/login"];
const adminAndTeacherLoginRoutes = ["/los_auth"];
...
async function middleware(req: NextRequest) {
  ...
  if (token) {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_APIURL}/teacher/checkJWT/?token=${token.value}`,
      {
        method: "GET",
        headers: {
  ...
  if (
    (accessAdminStatus === false &&
      protectedAdminRoutes.includes(req.nextUrl.pathname)) ||
    (accessTeacherStatus === false &&
      protectTeacherRoutes.includes(req.nextUrl.pathname)) ||
    (accessStudentStatus === false &&
      protectedStudentRoutes.includes(req.nextUrl.pathname))
  ) {
