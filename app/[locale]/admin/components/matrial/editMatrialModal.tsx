...
export default function EditorialModal({
  ...
  const [name, setName] = useState(martialDetails.name);
  const [age, setAge] = useState(martialDetails.age);
  const [course, setCourse] = useState(martialDetails.course);
  const [status, setStatus] = useState(martialDetails.status);
  ...
  const updateBook = () => {
    ...
    if (name !== martialDetails.name && name !== "") {
      updateData.name = name;
    }
    if (age !== martialDetails.age && age !== 0) {
      updateData.age = age;
    }
    if (course !== martialDetails.course && course !== "") {
      updateData.course = course;
    }
    if (status !== martialDetails.status) {
    ...
    fetch(`${process.env.NEXT_PUBLIC_APIURL}/material/${martialDetails.id}`, {
      method: "PATCH",
      headers: {
